...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self,
        ...
        last_oup_channels = last_inp_channels//3
        self.ResCBM = self._make_layer2(block=Res_CBAM_block,input_channels=last_inp_channels,output_channels=last_oup_channels,num_blocks=2)
        
        self.last_layer = nn.Sequential(
            nn.Conv2d(
                in_channels=last_oup_channels,
                out_channels=last_oup_channels,
                kernel_size=1,
                stride=1,
                padding=0),
            self.norm_layer(last_oup_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(
                in_channels=last_oup_channels,
                out_channels=1,
